...
class EntityTransaction extends EntityBase_1.EntityBase {
    ...
    static async mergeFind(storage, userId, ei, syncMap, trx) {
        ...
        let ef;
        if (ei.txid) {
            ef = (0, utilityHelpers_1.verifyOneOrNone)(await storage.findTransactions({ partial: { txid: ei.txid, userId }, trx }));
        }
        if (!ef && ei.reference) {
            ef = (0, utilityHelpers_1.verifyOneOrNone)(await storage.findTransactions({ partial: { reference: ei.reference, userId }, trx }));
        }
        return {
            found: !!ef,
